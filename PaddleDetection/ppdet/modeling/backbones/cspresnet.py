...
'''
这是一个基于飞桨框架的卷积层和批量归一化层的定义，该层被称为 ConvBNLayer。以下是该层的功能和参数的简要解释：

ch_in：输入通道数
ch_out：输出通道数
filter_size：卷积核尺寸，默认为 3x3
stride：步幅，默认为 1
groups：分组卷积参数，默认为 1，表示不使用分组卷积
padding：填充大小，默认为 0，表示不使用填充
act：激活函数，默认为 None，表示不使用激活函数，可以选择 ReLU、Sigmoid 等常见激活函数
在初始化方法中，ConvBNLayer 定义了一个 nn.Conv2D 层和一个 nn.BatchNorm2D 层。 nn.Conv2D 层实现了卷积操作，用于提取图像的特征。nn.BatchNorm2D 层是批量归一化操作，可以有效地加速模型训练和提高模型的泛化能力。

在前向传播方法中，输入数据 x 通过卷积层、批量归一化层和激活函数依次处理，并最终返回处理后的结果。
'''
