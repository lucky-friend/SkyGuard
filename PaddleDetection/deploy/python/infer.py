...
class Detector(object):
    ...
    def predict_video(self, video_file, camera_id):
        ...
        if camera_id != -1:
            ...
            capture1 = cv2.VideoCapture(video_file)
        ...
        encode_param=[int(cv2.IMWRITE_JPEG_QUALITY),80]#进行 JPEG 格式编码所需的参数
        async def send_msg(websocket):
            ...
            while (1):
                ...
                for i in range(box_num):
                    if box[i][1]>0.35:
                        ...
                        if(((x2-x1)*(x2-x1)+(y2-y1)*(y2-y1))>1600):
                            big_count+=1
                        elif(((x2-x1)*(x2-x1)+(y2-y1)*(y2-y1))<=1600 and ((x2-x1)*(x2-x1)+(y2-y1)*(y2-y1))>=1200):
                            mid_count+=1
                        else:
                            small_count+=1
                        ...
                        # # 添加观测值
                        # observations.append((cx, cy))
                        # # 如果列表的长度大于10，就删除最早的位置
                        # if len(observations) > 10:
                        #     observations.pop(0)

                        # # 如果有足够的观测值，开始预测
                        # if len(observations) >= 10:
                        #     # 使用第一个观测值初始化状态向量，速度初始化为0
                        #     kf.x = np.array([
                        #         [observations[0][0]],
                        #         [observations[0][1]],
                        #         [0],
                        ...
                        #         predicted_position = (predicted_state[0][0], predicted_state[1][0])
                        ...
                        #            [z[1]]
                        #         ]))
                        # next_position = kf.predict()[:2]
                        # # 计算运动方向
                        # dx = math.ceil(next_position[0][0])
                        # dy = math.ceil(next_position[1][0])
                        # print("Next position:", dx,dy)
                        # # 箭头长度
                        # arrow_length = 40

                        # # 计算箭头的方向向量
                        # arrow_direction = np.array([dx - cx, dy - cy])

                        # # 计算方向向量的单位向量
                        # arrow_unit_vector = arrow_direction / np.linalg.norm(arrow_direction)

                        # # 取反方向向量
                        # reversed_arrow_unit_vector = -arrow_unit_vector

                        # # 计算延长后的终点坐标
                        # extended_dx = int(cx + arrow_length * reversed_arrow_unit_vector[0])
                        # extended_dy = int(cy + arrow_length * reversed_arrow_unit_vector[1])
                        ...
                        # d1=(extended_dx-ex)*(extended_dx-ex)+(extended_dy-ey)*(extended_dy-ey)
                        # d2=(extended_dx-fx)*(extended_dx-fx)+(extended_dy-fy)*(extended_dy-fy)
                        # d3=(extended_dx-ex1)*(extended_dx-ex1)+(extended_dy-ey1)*(extended_dy-ey1)
                        # d4=(extended_dx-fx1)*(extended_dx-fx1)+(extended_dy-fy1)*(extended_dy-fy1)

                        # if(d1<=d2 and d1<=d3 and d1<=d4):
                        #     cv2.arrowedLine(frame, (cx, cy),(ex, ey), (0, 220, 100), 2, tipLength=0.3)
                        # elif(d2<=d1 and d2<=d3 and d2<=d4):
                        #     cv2.arrowedLine(frame, (cx, cy),(fx, fy), (0, 220, 100), 2, tipLength=0.3)
                        # elif(d3<=d1 and d3<=d2 and d3<=d4):
                        #     cv2.arrowedLine(frame, (cx, cy),(ex1, ey1), (0, 220, 100), 2, tipLength=0.3)
                        # else:
                        #     cv2.arrowedLine(frame, (cx, cy),(fx1, fy1), (0, 220, 100), 2, tipLength=0.3)
            ...
            #图像编码
                ...
                scores =big_count*8+mid_count*4+small_count*1
                msg_data = {
                    'image_origin' : "data:image/jpeg;base64,"+img1,
                    'image_predict': "data:image/jpeg;base64,"+img,
